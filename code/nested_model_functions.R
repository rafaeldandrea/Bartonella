...
dependent_parms = 
  function(parms, omega = NULL, xbar = NULL){
    ...
    if(delta_shape == 'V'){
      delta = min(delta) + (delta0 - min(delta)) / (max(delta) - delta0) * (max(delta) - delta)
    }
    ...
    if(gamma_shape == 'V'){
      gamma = min(gamma) + (gamma0 - min(gamma)) / (max(gamma) - gamma0) * (max(gamma) - gamma)
    }
...
evo_dynamics_fixedxbar_manual = 
  function(t, y, parms, evolve_interval, verbose){
    list2env(parms, envir = environment())
    ...
    while(time < max(t) & !is.na(min(S)) & !is.na(min(Iik))){
      ...
      if(time %% evolve_interval == 0){
        ...
        dwdt = (Ik > 1e-6) * colSums(beta_prime * S)
        
        omega = omega + dt[step] * dwdt
        omega = omega * (omega > 0)
