...
parameters =
  ...
  mutate(maxtime = ifelse(lomega < -.5, 20000, 2000)) %>%
...
if(do.manual_evolution){
  ...
  if(do.plots){
    ...
    result = 
      'c:/users/rdand/Google Drive/GitHub/Krishnada_project/data/20210928/parameter_exploration.rds' %>%
      readRDS()
    ...
    dtf = 
      ...
      slice_min(sigma_delta) %>%
      slice_min(sigma_gamma)
